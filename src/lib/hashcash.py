...
def make_token(s, n, charset='0123456789ABCDEF'):
    """Makes hashcash token of
    \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\] value 'n' against basestring 's'."""
    h = hashlib.sha256(s).hexdigest()
    for token in all_strings(charset):
        if trailing_zeros(hash((h + token).encode())) >= n: return token

def verify_token(s, token):
    """Returns hashcash value of given token against basestring 's'."""
    return trailing_zeros(hash((hashlib.sha256(s).hexdigest() + token).encode()))
