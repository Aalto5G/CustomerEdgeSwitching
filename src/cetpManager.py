...
class CETPManager:
    ...
    def _initiate_local_cetp(self):
        self.local_cetp = CETPH2H.CETPH2HLocal(l_cesid=self.cesid, policy_mgr=self.policy_mgr, cetp_security=self.cetp_security, \
                                               host_table=self.host_table, conn_table=self.conn_table, network=self.network, pool_table=self.pool_table)
    ...
    def create_cetp_endpoint(self, r_cesid, c2c_layer=None, c2c_negotiated=False):
        ...
        cetp_ep = CETPH2H.CETPH2H(l_cesid = self.cesid, r_cesid = r_cesid, cetpstate_table= self.cetpstate_table, policy_mgr=self.policy_mgr, network=self.network, \
    ...
    def process_outbound_cetp(self, dns_cb, cb_args, src_id, dst_id, r_cesid, naptr_list):
        ...
        try:
            if self.has_cetp_endpoint(r_cesid):
                ...
                ep.process_naptrs(src_id, dst_id, naptr_list, (dns_cb, cb_args))                            # Enqueues the NAPTR response and DNS-callback function.    # put_nowait() on queue will raise exception on a full queue.    - Use try: except:
            else:
